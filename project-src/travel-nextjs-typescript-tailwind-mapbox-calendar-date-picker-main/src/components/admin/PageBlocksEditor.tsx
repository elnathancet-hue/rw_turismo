...
const PageBlocksEditor = ({ blocks, onChange }: Props) => {
  // Merge a partial patch into the block at `index`. The union is handled by
  // the block-type guards in the JSX, so a loose patch type is fine here.
  ...
  const add = (type: PageBlock["type"]) => onChange([...blocks, createBlock(type)]);
  ...
  return (
    <div className="space-y-4">
      ...
      {blocks.map((block, index) => (
        <Card className="space-y-4 p-4" key={block.id}>
