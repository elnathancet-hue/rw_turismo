...
const FeaturedProducts = ({ section, products }: { section: HomeSection; products: Product[] }) => {
  const selected = Array.isArray(section.content?.product_ids)
    ? products.filter((product) => section.content.product_ids.includes(product.id))
    : products;
  const items = selected.slice(0, Number(section.content?.limit ?? 6));
  return (
