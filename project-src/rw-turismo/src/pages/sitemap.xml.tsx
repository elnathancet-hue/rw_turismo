...
import { getPublicBlogTaxonomy, getPublishedPageSlugs, getPublishedPosts } from "../lib/content/server";
...
export const getServerSideProps:GetServerSideProps=async({res})=>{
  ...
  const [productsResult, blogResult, taxonomyResult, pagesResult] = await Promise.allSettled([
  ...
  if (taxonomyResult.status === "fulfilled") {
    ...
    taxonomyResult.value.tags.forEach(item=>urls.push({loc:`${base}/blog/tag/${item.slug}`}));
  }
  if (pagesResult.status === "fulfilled") {
    pagesResult.value.forEach(item=>urls.push({loc:`${base}/paginas/${item.slug}`,lastmod:item.updated_at}));
  }
